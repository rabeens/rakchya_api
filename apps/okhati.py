...
def okhati_request(user_id, okhatiFile):
    ...
    try:
        ...
        cursor.execute("Insert into okhati_requests (invoice_asset_path, client_id) values(%s, %s)", user_data)
        res = {
            "msg" : "Invoice added",
            "status":200
        }
...
def okhati_my_request(user_id , limit = 20, offset = 0):
    ...
    try:
        ...
        cursor.execute('''select 
            ...
            from okhati_requests where client_id = %s and is_active = 'Y' and is_closed = 'N' order by requested_date desc limit %s, %s''', (user_id, offset, limit ))
        res = {
...
def okhati_my_request_image(user_id, request_id, limit = 20, offset = 0):
    ...
    try:
        ...
        cursor.execute('''select 
            invoice_asset_path
            from okhati_requests where request_id = %s and  client_id = %s and is_active = 'Y'  order by requested_date desc limit %s, %s''', (request_id, user_id, offset, limit ))
...
# Not required. Cann directly access the static files inside IP:POST/static/resources/invoice_sample.pdf
